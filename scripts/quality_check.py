"""Graph Quality Checker - identifies issues in the knowledge graph."""
...
async def quality_check():
    ...
    async with GraphBuilder.from_settings(settings.falkordb) as builder:
        ...
        console.print("\n[bold]Check 5: Orphaned Nodes[/bold]")
        orphans = await _check_orphaned_nodes(builder)
        for node_type, count in orphans.items():
            if count > 0:
                console.print(f"  [red]✗[/red] {node_type}: {count} orphaned nodes")
                issues.append(f"Orphaned {node_type}: {count}")
            else:
                console.print(f"  [green]✓[/green] {node_type}: No orphans")

        # === Check 6: Data Completeness ===
        console.print("\n[bold]Check 6: Data Completeness[/bold]")
        completeness = await _check_data_completeness(builder)
        for metric, (present, total, pct) in completeness.items():
            color = "green" if pct > 80 else "yellow" if pct > 50 else "red"
            console.print(f"  [{color}]{metric}: {present}/{total} ({pct:.1f}%)[/{color}]")
...
async def _check_duplicates(builder):
    ...
    # StockPrice: check by (symbol, date) - CORRECT composite key
    rows = await builder.query_raw(
        "MATCH (n:StockPrice) "
...
async def _check_missing_relationships(builder):
...
async def _check_subsidiaries(builder):
    """Check subsidiary data quality."""
    issues = []

    # Companies with subsidiaries in vnstock but not in graph
    rows = await builder.query_raw(
        "MATCH (c:Company) "
        "WHERE NOT (c)<-[:SUBSIDIARY_OF]-(:Company) "
        "AND NOT (c)-[:SUBSIDIARY_OF]->(:Company) "
        "RETURN count(c)"
    )
    # Note: This is just a placeholder - we'd need external data to compare
...
async def _check_shareholder_classification(builder):
    ...
    rows = await builder.query_raw(
        ...
        "RETURN count(p), collect(p.name)[..5]"
    )
    if rows and rows[0][0] > 0:
        issues.append(f"Corporate shareholders classified as Person: {rows[0][0]} (e.g. {rows[0][1]})")
...
async def _check_orphaned_nodes(builder):
    """Check for orphaned nodes (sample-based, avoids timeout)."""
    results = {}
    for label in ["StockPrice", "Indicator", "FinancialStatement", "Date", "Quarter"]:
        # Use a more efficient query - check if ANY orphan exists
        rows = await builder.query_raw(
            f"MATCH (n:{label}) WHERE NOT (n)--() RETURN count(n) LIMIT 1"
        )
        results[label] = rows[0][0] if rows else 0
    return results


async def _check_data_completeness(builder):
    ...
    rows = await builder.query_raw(
        "MATCH (c:Company) "
        "OPTIONAL MATCH (c)-[:HAS_STOCK_PRICE]->(p:StockPrice) "
        "WITH c, count(p) AS price_count "
        "RETURN count(CASE WHEN price_count > 0 THEN 1 END) as with_prices, count(c) as total"
    )
    if rows:
        results["Companies with price data"] = (
            rows[0][0],
            rows[0][1],
...
if __name__ == "__main__":
    issues = asyncio.run(quality_check())
